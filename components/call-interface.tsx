...
export function CallInterface({ scenario, phoneNumber, onCallComplete, onBack }: CallInterfaceProps) {
  ...
  const initiateCall = async () => {
    ...
    try {
      ...
      try {
        console.log("Attempting real phone call via Omnidimension API...")
        ...
        toast({
          ...
          description: "Using simulation mode for demonstration. This will show how the real system works.",
          variant: "destructive",
        })
  ...
  const startStatusChecking = (callId: string) => {
    statusCheckInterval.current = setInterval(async () => {
      try {
        ...
        if (status.status === "connected" && callStatus !== "connected") {
        ...
        {apiConnectionFailed && (
          <Alert className="mb-6 bg-yellow-500/10 border-yellow-500/20">
            ...
            <AlertDescription className="text-white/80">
              <strong>Demo Mode:</strong> The real phone service is currently unavailable. This demonstration shows how
              the system works with simulated calls and AI analysis.
            </AlertDescription>
          </Alert>
        )}
